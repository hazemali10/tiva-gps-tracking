...
void GPS_read() {
    ...
    do {
        while (UART2_getChar() != GPS_logName[i]) 
        {
    ...
    while (1) {
        ...
        if (flag) { //if the received character is a comma it means the end of a data field has been reached so the counter is incremented.
            recievedChar = UART2_getChar(); // after each char flag = 1 to increment counter  
            if (recievedChar == ',') {
                counter++;
            }
        }
    ...
    if(valid == 'A') { // Valid
        if(NS =='N')
            currentLat = atof(latitude)/100;
        else
            currentLat = -atof(latitude)/100;
        
        if(EW =='E') 
            currentLong = atof(longitude)/100;
        else
            currentLong = -atof(longitude)/100;
    }
}
